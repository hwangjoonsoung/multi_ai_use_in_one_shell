...
impl Grid {
    ...
    pub fn scroll_up(&mut self, count: u16) {
        for _ in 0..(count.min(self.size.rows - self.scroll_top)) {
            ...
            // MAI: top-anchored partial regions (e.g. Codex history above its
            // composer) also evict real terminal history. Interior regions do not.
            if self.scrollback_len > 0
                && (!self.scroll_region_active() || self.scroll_top == 0)
            {
