...
public final class WindowsProcessLauncher implements ProcessLauncher {
    ...
    public RunOutcome run(Invocation inv) throws InterruptedException {
        ...
        // SPEC §9.2 Q5 — Claude Code 세션 식별 환경변수를 자식에게 물려주지 않는다.
        pb.environment().keySet().removeIf(WindowsProcessLauncher::isSessionMarker);
        ...
        try {
            ...
            Future<String> fo = pumps.submit(() -> drain(p.getInputStream(), outF));
            Future<String> fe = pumps.submit(() -> drain(p.getErrorStream(), errF));
     ...
     * taskkill /T 도 쓸 수 없다 (교차검토 R5·C3).
     *
     * @return 종료를 확인하지 못한 생존 PID. 관측·추적한 핸들에 한정된다.
     */
    private List<Long> terminateTree(Process parent) {
        ...
        if (ph.isAlive()) {
            if (!taskkillTree(ph.pid())) parent.destroyForcibly();
    ...
    /** Windows 내장 유틸리티. 외부 Java 라이브러리가 아니므로 D12 를 위반하지 않는다. */
    private boolean taskkillTree(long pid) {
        try {
            Process k = new ProcessBuilder("taskkill", "/PID", Long.toString(pid), "/T", "/F")
    ...
    private static boolean isSessionMarker(String key) {
