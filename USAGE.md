...
```
multi-ai(20260903-210657)> 로그인 설계에서 빠진 위험을 찾아줘

[Claude · 실행 중] [Codex · 실행 중] [Gemini via agy · 실행 중]
...
[Gemini via agy] 성공  11.1s

...
```

세 답변을 한꺼번에 쏟으면 읽을 수가 없다. 그래서 **요약표만 먼저** 나온다.

```
  ────────────────────────────────────────────────────────────
  [1] Claude            성공    5.6s  1,240자
  [2] Codex             성공    6.7s  2,310자
  [3] Gemini via agy    실패    0.2s  —
  ────────────────────────────────────────────────────────────
  /v <번호>  답변 보기   ·   /v all  전체   ·   /v  마지막 요약 다시
```
...
한 답변은 굵은 선으로 위아래를 막아 다른 답변과 섞이지 않는다.
...
- 한 AI가 실패해도 **나머지 응답은 그대로 보존**된다.
- 라운드가 도는 동안 미리 쳐둔 입력은 **버려지지 않고** 끝난 뒤 순서대로 처리된다.
...
| `/preset rm <이름>` | 프리셋 삭제 |
| `/v <번호\|이름\|all>` | **답변 구역 펼쳐 보기.** 인자 없으면 요약표 |
